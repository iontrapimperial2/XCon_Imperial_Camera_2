...
#            print("[%s]: %s" %(function, error))
    ...
    def SaveAsTxt2(self, path, n):
        file = open(path, 'w')
        width = 512 #int(np.sqrt(len(self.imageArray)))
        for k in range(int(n)*width):
    ...
    def GetAvailableCameras(self):
        cameras = c_long()
        error = self.dll.GetAvailableCameras(byref(cameras))
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        self.availablecamera = cameras.value
        return ERROR_CODE[error]
        
    def GetCameraHandle(self,CameraIndex):
        CameraHandle = c_long()
        error = self.dll.GetCameraHandle(CameraIndex,byref(CameraHandle))
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
...
#        self.camerahandle = CameraHandle.value
    ...
    def GetCurrentCamera(self):
        CurrentCamera = c_long()
        error = self.dll.GetCurrentCamera(byref(CurrentCamera))
        self.verbose(ERROR_CODE[error], sys._getframe().f_code.co_name)
        self.currentcamera = CurrentCamera.value
        return ERROR_CODE[error]
